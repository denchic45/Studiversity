...
class FindEventsOfWeekByThisUserUseCase(private val eventRepository: EventRepository) {
    ...
    private fun testData(): Flow<List<EventsOfDay>> {
        ...
        val physicalCultureEvent = Event("",
            groupHeader,
            details = Lesson(Subject("", "Физкультура", "ic_basketball"), listOf()))

        val emptyEvent = Event("", groupHeader, details = EmptyEventDetails())

        return flowOf(listOf(EventsOfDay(LocalDate.parse("2022-09-05"),
            listOf(physicalCultureEvent,
