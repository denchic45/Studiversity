...
abstract class SubmissionDao : BaseDao<SubmissionEntity>() {
    ...
    @Query("SELECT * FROM user u INNER  JOIN group_course gc ON u.user_group_id = gc.group_id INNER JOIN submission s ON s.student_id != u.user_id AND s.course_id = gc.course_id WHERE s.content_id=:taskId")
