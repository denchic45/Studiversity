...
abstract class SubjectDao : BaseDao<SubjectEntity>() {
    ...
    abstract suspend fun get(subjectId: String): SubjectEntity?

    @Query("SELECT * FROM subject WHERE subject_id =:subjectId")
    abstract fun observe(subjectId: String): Flow<SubjectEntity?>
    ...
    abstract fun observeByGroupId(groupId: String): Flow<List<SubjectEntity>>
